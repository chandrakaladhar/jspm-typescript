...
exports.bundle = function (loads, compileOpts, outputOpts) {
    ...
    var globals;
    if (outputOpts.less && outputOpts.less.length) {
        return Promise.all(outputOpts.less.map((lessConfig)=> {
            if (lessConfig.globals) globals = '\r\n' + lessConfig.globals;
            return less.render(lessOutput + globals, {
                compress: false,
                sourceMap: outputOpts.sourceMaps
            })
        })).then(function (dataArr) {
            var retOut = stubDefines;
            dataArr.forEach((data, i)=> {
                var cssOutput = data.css;
                var outFile = loader.separateCSS ? outputOpts.outFile.replace(/\.js$/, outputOpts.less[i].extn) : rootURL;
                ...
                if (loader.separateCSS) {
                    ...
                    retOut = [stubDefines, cssInject, '("' + escape(cssOutput) + '");'].join('\n');
                }
            });
            return retOut;
        }).catch(function (e) {
...
exports.listAssets = function (loads, compileOpts, outputOpts) {
    ...
    return Promise.all(lessOutput.map(function (load) {
        var globals;
        if (outputOpts.less && outputOpts.less.length && outputOpts.less[0].globals) {
            globals = '\r\n' + outputOpts.less[0].globals;
        }
        return less.render(load.source + globals, {
            compress: false,
            sourceMap: outputOpts.sourceMaps
        }).then(function (data) {
            return {
